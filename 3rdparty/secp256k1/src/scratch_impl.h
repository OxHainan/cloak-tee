...
static secp256k1_scratch* secp256k1_scratch_create(
  const secp256k1_callback* error_callback, size_t max_size)
{
    secp256k1_scratch* ret =
      (secp256k1_scratch*)checked_malloc(error_callback, sizeof(*ret));
    if (ret != NULL)
    {
...
static int secp256k1_scratch_allocate_frame(
  secp256k1_scratch* scratch, size_t n, size_t objects)
{
    ...
    {
        n += objects * ALIGNMENT;
        scratch->data[scratch->frame] =
          checked_malloc(scratch->error_callback, n);
...
static void* secp256k1_scratch_alloc(secp256k1_scratch* scratch, size_t size)
{
    ...
    if (
      scratch->frame == 0 ||
      size + scratch->offset[frame] > scratch->frame_size[frame])
    {
        return NULL;
    }
    ret =
      (void*)((unsigned char*)scratch->data[frame] + scratch->offset[frame]);
    memset(ret, 0, size);
