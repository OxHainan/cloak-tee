  ...
  const secp256k1_ecdsa_recoverable_signature* sig)
{
    ...
    if (sizeof(secp256k1_scalar) == 32)
    {
         ...
         * representation inside secp256k1_ecdsa_signature, as conversion is
         * very fast. Note that secp256k1_ecdsa_signature_save must use the same
         * representation. */
  ...
  int recid)
{
    ...
    {
        if (
          secp256k1_fe_cmp_var(&fx, &secp256k1_ecdsa_const_p_minus_order) >= 0)
        {
            return 0;
        }
  ...
  const void* noncedata)
{
    ...
    if (!overflow && !secp256k1_scalar_is_zero(&sec))
    {
        ...
        {
            ret =
              noncefp(nonce32, msg32, seckey, NULL, (void*)noncedata, count);
            if (!ret)
            {
                break;
            }
            ...
            if (!secp256k1_scalar_is_zero(&non) && !overflow)
            {
                if (secp256k1_ecdsa_sig_sign(
                      &ctx->ecmult_gen_ctx, &r, &s, &sec, &msg, &non, &recid))
                {
                    break;
                }
            }
  ...
  const unsigned char* msg32)
{
    ...
    VERIFY_CHECK(
      recid >= 0 && recid < 4); /* should have been caught in parse_compact */
