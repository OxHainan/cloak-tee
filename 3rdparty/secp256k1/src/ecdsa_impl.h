...
static int secp256k1_der_read_len(
  const unsigned char** sigp, const unsigned char* sigend)
{
    int lenleft, b1;
    size_t ret = 0;
    if (*sigp >= sigend)
...
static int secp256k1_der_parse_integer(
  secp256k1_scalar* r, const unsigned char** sig, const unsigned char* sigend)
{
    int overflow = 0;
...
static int secp256k1_ecdsa_sig_parse(
  secp256k1_scalar* rr,
  secp256k1_scalar* rs,
...
static int secp256k1_ecdsa_sig_verify(
  ...
  const secp256k1_scalar* message)
{
     ...
     *    <=> (xr * pr.z^2 mod p == pr.x) || (xr + n < p && (xr + n) * pr.z^2
     * mod p == pr.x)
     *
  ...
  int* recid)
{
    ...
    if (recid)
    {
         ...
         * requires finding the discrete log of some P where P.x >= order, and
         * only 1 in about 2^127 points meet this criteria.
         */
