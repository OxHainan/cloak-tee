    ...
    struct Function
    {
     public:
        ...
        std::vector<std::string> get_mapping_keys(
            const std::string& msg_sender, const std::string& name, int pos = -1, bool encoded = true)
        {
            std::vector<std::string> res;
            auto ps = read;
            ps.insert(ps.end(), mutate.begin(), mutate.end());
            LOG_DEBUG_FMT("get_mapping_keys, msg_sender:{}, name:{}", msg_sender, name);
            for (auto&& x : ps) {
                if (x.name == name) {
                    for (auto&& key : x.keys) {
                        std::vector<std::string> nested_keys = Utils::split_string(key, ':');
                        LOG_DEBUG_FMT("key:{}, nested_keys:{}", key, fmt::join(nested_keys, ", "));
                        if (pos != -1) {
                        ...
                        for (auto&& single_key : nested_keys) {
                            ...
                            for (auto&& input : inputs) {
                                if (input.name == single_key) {
                                    if (encoded) {
                                        auto data =
                                            abicoder::Encoder::encode("", input.value.value(), input.structural_type);
...
struct formatter<evm4ccf::Status>
{
    ...
    auto format(const evm4ccf::Status& status, FormatContext& ctx) -> decltype(ctx.out())
    {
        switch (status) {
