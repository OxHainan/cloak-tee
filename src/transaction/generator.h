    ...
    class Generator
    {
      ...
      public:
        ...
        void sync_states(const SyncStates& syncStates)
        {
            ...
            {
                throw TransactionException(fmt::format(
                  "function is not ready, get {}", syncStates.tx_hash));
            }
            ...
            if (!cp_opt->request_public_keys(
                  syncStates.tx_hash, acc, service_addr))
            {
                auto new_states = Ethereum::execute_mpt(
                  ctx, cp_opt.value(), acc->get_address(), old_states);
                sync_result(
                  syncStates.tx_hash, cp_opt.value(), acc, new_states);
            }
      ...
      private:
        std::tuple<eevm::Keccak256, PrivacyPolicyTransaction>
        ...
        std::vector<std::string> get_proof(
          CloakPolicyTransaction& cpt, const eevm::Keccak256& target_digest)
        {
            auto pd = ctx.tx.rw(tables.privacy_digests);
            ...
            return {
              cpt.codeHash,
              privacy_digest.value().hex_str(),
              old_states_digest.value().hex_str()};
        }
    };
