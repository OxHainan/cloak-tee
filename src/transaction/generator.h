...
namespace cloak4ccf
{
    namespace Transaction
    {
        ...
        class Generator
        {
          ...
          public:
            explicit Generator(CloakContext& ctx_) :
              ctx(ctx_),
              tables(ctx.cloakTables.txTables)
            {}

            auto add_privacy(const eevm::rlp::ByteString& encoded)
            {
                const auto decoded =
                  evm4ccf::PrivacyTransactionWithSignature(encoded);
                ...
                {
                    LOG_DEBUG_FMT(
                      "privacy digests already exists (digests {})",
                ...
                pd->put(tc.to, hash);
                LOG_INFO_FMT(
                  "add privacy digests {}, address {}",
                  eevm::to_hex_string(hash),
                  eevm::to_hex_string(tc.to));
                return hash;
            }
            ...
            {
                const auto decoded =
                  evm4ccf::CloakTransactionWithSignature(encoded);
                ...
                {
                    eevm::KeccakHash target_digest =
                      Utils::vec_to_KeccakHash(mpt.to);
                ...
                // check nonce
                auto es = Ethereum::EthereumState::make_state(
                  ctx.tx, ctx.cloakTables.acc_state);
                ...
                LOG_INFO_FMT(
                  "add user transaction digests {}",
                  eevm::to_hex_string(multi_digest));
            ...
            void sync_public_keys(const SyncKeys& syncKeys)
            {
                ...
                auto public_key_list =
                  abicoder::Decoder::decode_bytes_array(public_keys_data);

                for (size_t i = 0; i < cp_opt->requested_addresses.size(); i++)
                {
                    public_keys[cp_opt->requested_addresses[i]] =
                      public_key_list[i];
                }
                ...
                auto new_states = Ethereum::execute_mpt(
                  ctx, cp_opt.value(), acc->get_address(), decrypted);
            ...
            void sync_states(const SyncStates& syncStates)
            {
                ...
                auto service_addr = TeeManager::get_service_addr(
                  ctx.tx.rw(ctx.cloakTables.tee_table.service));
                if (!cp_opt->request_public_keys(
                      target_digest, acc, service_addr))
                {
                    auto new_states = Ethereum::execute_mpt(
                      ctx, cp_opt.value(), acc->get_address(), old_states);
                    sync_result(target_digest, cp_opt.value(), acc, new_states);
                }
          ...
          private:
            std::tuple<eevm::KeccakHash, PrivacyPolicyTransaction>
            check_privacy_modules(const eevm::Address& to)
            {
                auto pd = ctx.tx.rw(tables.privacy_digests);
                auto p = ctx.tx.rw(tables.privacys);
                ...
                {
                    throw TransactionException(fmt::format(
                      "privacy digests doesn`t exists (contract address {})",
              ...
              TeeManager::AccountPtr acc)
            {
                ...
                auto response = Ethereum::SyncStateResponse(
                  target_digest, acc->get_address(), cpt.verifierAddr, data);
              ...
              TeeManager::AccountPtr acc)
            {
                ...
                Ethereum::MessageCall mc(
                  acc->get_address(), service_addr, packed);
                auto signed_data =
                  evm4ccf::sign_eth_tx(acc->get_tee_kp(), mc, acc->get_nonce());
                auto response =
                  Ethereum::SyncStateResponse(target_digest, signed_data);
            ...
            {
                auto new_states =
                  abicoder::Decoder::decode_bytes_array(new_states_);
                LOG_DEBUG_FMT(
                  "splited new_states:{}\n", fmt::join(new_states, "\n"));

                auto encrypted_states =
                  cpt.encrypt_states(acc->get_tee_kp(), new_states);
                LOG_DEBUG_FMT(
                  "encrypted:{}", fmt::join(encrypted_states, ", "));
                ...
                encoder.add_inputs(
                  "proof",
                  "uint256[]",
                  get_proof(cpt, target_digest),
                  abicoder::make_number_array());
                auto packed = encoder.encodeWithSignatrue();
                LOG_DEBUG_FMT(
                  "encoded data:{}", abicoder::split_abi_data_to_str(packed));
                ...
                Ethereum::MessageCall mc(
                  acc->get_address(),
                  service_addr,
                  encoderCom.encodeWithSignatrue());
            ...
            std::vector<std::string> get_proof(
              CloakPolicyTransaction& cpt, evm4ccf::h256& target_digest)
            {
                auto pd = ctx.tx.rw(tables.privacy_digests);
