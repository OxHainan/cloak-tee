...
#    include "virtual_host.h"
        ...
        void* time_location)
    {
        ...
        auto writer_factory = std::make_unique<
            oversized::WriterFactory>(*basic_writer_factory, ec.writer_config);
...

        {
            ...
            *enclave_version_len = ccf_version_string.size();
            ...
            if (!ccf::Pal::is_outside_enclave(
                    ...
                    ec.from_enclave_buffer_size)) {
                LOG_FAIL_FMT(
                    "Memory outside enclave: from_enclave buffer start");
                return CreateNodeStatus::MemoryNotOutsideEnclave;
            }
            ...
            if (!ccf::Pal::is_outside_enclave(
                ...
                LOG_FAIL_FMT(
                    "Memory outside enclave: to_enclave buffer offset");
                return CreateNodeStatus::MemoryNotOutsideEnclave;
            }
            ...
            if (!ccf::Pal::is_outside_enclave(
                    ...
                    sizeof(ringbuffer::Offsets))) {
                LOG_FAIL_FMT(
                    "Memory outside enclave: from_enclave buffer offset");
                return CreateNodeStatus::MemoryNotOutsideEnclave;
            }
        ...
        // enclaves
        if (cc.start.service_configuration.reconfiguration_type.has_value() &&
            cc.start.service_configuration.reconfiguration_type.value() !=
                ReconfigurationType::ONE_TRANSACTION) {
            LOG_FAIL_FMT(
                "2TX reconfiguration is experimental, disabled in release "
                "mode");
        ...
        try {
            ...
            LOG_FAIL_FMT(
                "ccf_openssl_rdrand_init_error during enclave init: {}",
                e.what());
        ...
        if (e.load() != nullptr) {
            uint16_t tid;
            {
                ...
                threading::thread_ids.emplace(
                    std::pair<
                        std::thread::id,
                        uint16_t>(std::this_thread::get_id(), tid));
