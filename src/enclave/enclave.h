 ...
 public:
      ...
      rpcsessions(std::make_shared<RPCSessions>(*writer_factory, rpc_map))
    {
        ...
        rpc_map->register_frontend<ccf::ActorsType::members>(
            std::make_unique<
                ccf::MemberRpcFrontend>(network, *context, share_manager));
    ...
    {
        // node_cert_size and service_cert_size are ignored here, but we pass
        // them in because it allows us to set EDL an annotation so that
        // node_cert_len
        ...
        try {
            LOG_TRACE_FMT(
                "Creating node with start_type {}",
                start_type_to_str(start_type));
        ...
        *node_cert_len = r.self_signed_node_cert.size();
        ...
        if (start_type == StartType::Start ||
            ...
            // When starting a node in start or recover modes, fresh network
            // secrets are created and the associated certificate can be passed
            // to the host
    ...
    bool run_main()
    {
...
#endif
        {
            ...
            DISPATCHER_SET_MESSAGE_HANDLER(
                ...
                [this, &disp = bp.get_dispatcher()](const uint8_t*, size_t) {
                    ...
                    if (elapsed_ms.count() > 0) {
                        ...
                        threading::ThreadMessaging::thread_messaging.tick(
                            elapsed_ms);
                        // When recovering, no signature should be emitted while
                        // the public ledger is being read
                        if (!node->is_reading_public_ledger()) {
                            for (auto& [actor, frontend] :
                                 rpc_map->frontends()) {
            ...
            DISPATCHER_SET_MESSAGE_HANDLER(
                ...
                [this](const uint8_t* data, size_t size) {
                    const auto [from_seqno, to_seqno, purpose, body] =
                        ringbuffer::read_message<
                            consensus::ledger_entry_range>(data, size);
                    switch (purpose) {
                        case consensus::LedgerRequestPurpose::Recovery:
                        {
                            if (node->is_reading_public_ledger() ||
                                ...
                                LOG_FAIL_FMT(
                                    "Cannot recover ledger entry: Unexpected "
                                    "node state {}",
                                    s);
                            }
                            break;
            ...
            DISPATCHER_SET_MESSAGE_HANDLER(
                ...
                [this](const uint8_t* data, size_t size) {
                    const auto [from_seqno, to_seqno, purpose] =
                        ringbuffer::read_message<
                            consensus::ledger_no_entry_range>(data, size);
                    switch (purpose) {
                        case consensus::LedgerRequestPurpose::Recovery:
                        {
            ...
            while (!bp.get_finished()) {
                ...
                auto read =
                    bp.read_n(max_messages, circuit->read_from_outside());
                ...
                if (read == 0 && thread_msg == 0) {
                    ...
                    // Handle initial idles by pausing, eventually sleep (in
                    // host)
            ...
            // messages catch any thrown exception they can recover from.
            // Uncaught exceptions bubble up to here and cause the node to
            // shutdown.
            RINGBUFFER_WRITE_MESSAGE(
    ...
    bool run_worker()
    {
...
#endif
        {
            ...
            threading::ThreadMessaging::thread_messaging
                .add_task(msg->data.tid, std::move(msg));
