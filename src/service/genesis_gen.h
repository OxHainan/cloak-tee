...
class GenesisGenerator
{
 ...
 public:
    ...
    void activate_member(const MemberId& member_id)
    {
        ...
        if (!member.has_value()) {
            throw std::logic_error(fmt::format(
                "Member {} cannot be activated as they do not exist",
                member_id));
        }

        member->status = MemberStatus::ACTIVE;
        if (is_recovery_member(member_id) &&
            (get_active_recovery_members().size() >=
             max_active_recovery_members)) {
            throw std::logic_error(fmt::format(
                "Cannot activate new recovery member {}: no more than {} "
                "active "
    ...
    bool remove_member(const MemberId& member_id)
    {
        ...
        if (!member_to_remove.has_value()) {
            // The remove member proposal is idempotent so if the member does
            // not exist, the proposal should succeed with no effect
            LOG_FAIL_FMT(
        ...
        if (member_to_remove->status == MemberStatus::ACTIVE &&
            ...
            if (get_active_recovery_members_count_after < recovery_threshold) {
                LOG_FAIL_FMT(
                    "Failed to remove recovery member {}: number of active "
                    "recovery "
            ...
            [&active_nodes,
             self_to_exclude](const NodeId& nid, const NodeInfo& ni) {
                if ((ni.status == ccf::NodeStatus::TRUSTED ||
                    ...
                    (!self_to_exclude.has_value() ||
                     self_to_exclude.value() != nid)) {
                    active_nodes[nid] = ni;
                }
                return true;
            });
        ...
        bool recovering = false)
    {
        ...
        if (service->has()) {
            ...
            auto previous_service_identity =
                tx.wo<ccf::PreviousServiceIdentity>(
        ...
        service->put(
            {service_cert,
             ...
             recovering ? service->get_version_of_previous_write() :
                          std::nullopt,
             recovery_count,
             service_data,
             create_txid});
    ...
    bool open_service()
    {
        ...
        if (active_recovery_members_count < get_recovery_threshold()) {
            LOG_FAIL_FMT(
                "Cannot open network as number of active recovery members ({}) "
                "is "
    ...
    void init_configuration(const ServiceConfiguration& configuration)
    {
        ...
        if (config->has()) {
            throw std::logic_error(
                "Cannot initialise service configuration: configuration "
                "already "
                "exists");
        }
    ...
    bool set_recovery_threshold(size_t threshold)
    {
        ...
        if (service_status.value() ==
            ServiceStatus::WAITING_FOR_RECOVERY_SHARES) {
            // While waiting for recovery shares, the recovery threshold cannot
            // be modified. Otherwise, the threshold could be passed without
            // triggering the end of recovery procedure
            LOG_FAIL_FMT(
                "Cannot set recovery threshold: service is currently waiting "
                "for "
                "recovery shares");
            return false;
            ...
            if (threshold > get_active_recovery_members_count) {
                LOG_FAIL_FMT(
                    "Cannot set recovery threshold to {} as it is greater than "
                    "the "
    ...
    size_t get_recovery_threshold()
    {
        ...
        if (!current_config.has_value()) {
            throw std::logic_error(
                "Failed to get recovery threshold: No active configuration "
                "found");
        }
