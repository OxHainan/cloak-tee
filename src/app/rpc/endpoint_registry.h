    ...
    class EVMHandlers : public AbstractEndpointRegistry
    {
      private:
        void install_standard_rpcs()
        {
            ...
            auto get_gasPrice = [](CloakContext&, const nlohmann::json&) {
                return 0;
            };
            ...
            auto send_raw_transaction =
              [this](CloakContext& ctx, const nlohmann::json& params) {
                  ...
                  auto es = make_state(ctx.tx);
                  auto tx_result =
                    Ethereum::EVMC(tc, es, ctx.tx.rw(tables.tx_results)).run();
            ...
            make_endpoint(
              ...
              HTTP_POST,
              json_adapter(get_chainId, tables),
              auth_policies)
              .install();
            make_endpoint(
              Ethereum::ethrpc::GetGasPrice::name,
              HTTP_POST,
              json_adapter(get_gasPrice, tables),
              auth_policies)
              .install();
            ...
            make_endpoint(
              ...
              HTTP_POST,
              json_adapter(get_balance, tables),
              auth_policies)
              .install();
            ...
            make_endpoint(
              "eth_sendPop",
              HTTP_POST,
              json_adapter(send_pop, tables),
              auth_policies)
              .install();
            make_endpoint(
              "eth_sendSetup",
              HTTP_POST,
              json_adapter(send_setup, tables),
              auth_policies)
              .install();
            make_endpoint(
              "eth_getProposal",
              HTTP_POST,
              json_adapter(get_proposal, tables),
              auth_policies)
              .install();
        }
