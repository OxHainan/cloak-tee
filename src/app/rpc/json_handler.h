...
namespace cloak4ccf
{
    ...
    static void set_response(
      JsonAdapterResponse&& res, std::shared_ptr<enclave::RpcContext>& ctx)
    {
...
#pragma clang diagnostic ignored "-Wunused-function"
    ...
    using HandlerJsonParamsAndForward = std::function<JsonAdapterResponse(
      CloakContext& ctx, nlohmann::json params)>;

    static ccf::endpoints::EndpointFunction json_adapter(
      const HandlerJsonParamsAndForward& f, CloakTables& table)
    {
        return [f, &table](ccf::endpoints::EndpointContext& args) {
            ...
            JsonAdapterResponse result =
              func([&]() { return f(ctx, params); }, ctx);
            set_response(std::move(result), args.rpc_ctx);
        };
    }

    using ReadOnlyHandlerWithJson = std::function<JsonAdapterResponse(
      ReadOnlyCloakContext& ctx, nlohmann::json& params)>;
