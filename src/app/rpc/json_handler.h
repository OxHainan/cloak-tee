...
template <typename T>
static JsonAdapterResponse make_error(const T& ctx,
                                      http_status status,
...
static void set_response(JsonAdapterResponse&& res, std::shared_ptr<enclave::RpcContext>& ctx) {
    ...
    if (error != nullptr) {
        ...
        ctx->set_response_header(http::headers::CONTENT_TYPE,
...
using HandlerJsonParamsAndForward =
    std::function<JsonAdapterResponse(CloakContext& ctx, nlohmann::json params)>;

static ccf::EndpointFunction json_adapter(const HandlerJsonParamsAndForward& f,
                                          CloakTables& table) {
...
using ReadOnlyHandlerWithJson =
    std::function<JsonAdapterResponse(ReadOnlyCloakContext& ctx, nlohmann::json& params)>;

static ccf::ReadOnlyEndpointFunction json_read_only_adapter(const ReadOnlyHandlerWithJson& f,
                                                            CloakTables& table) {
