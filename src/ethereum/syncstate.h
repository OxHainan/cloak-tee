...
struct SyncStateResponse {
    ...
    SyncStateResponse(const eevm::KeccakHash& hash, const std::vector<uint8_t>& data_) :
        tx_hash(hash), data(data_) {}

    SyncStateResponse(const eevm::KeccakHash& hash,
                      const eevm::Address& from_,
                      const eevm::Address& to_,
                      const std::vector<uint8_t>& data_) :
        tx_hash(hash),
        data(data_), from(from_), to(to_) {}
...
inline void to_json(nlohmann::json& j, const SyncStateResponse& s) {
    ...
    j["tx_hash"] = eevm::to_hex_string(s.tx_hash);
...
inline void from_json(const nlohmann::json& j, SyncStateResponse& s) {
    ...
    s.tx_hash = Utils::to_KeccakHash(j["tx_hash"]);
