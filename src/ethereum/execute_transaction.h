...
namespace Ethereum
{
      ...
      protected:
        ...
        std::tuple<eevm::ExecResult, TxHash, Address> run_in_evm()
        {
            ...
            return std::make_tuple(
              exec_result, hash, account_state.acc.get_address());
        }
    ...
    std::vector<uint8_t> execute_mpt(
      ...
      const std::vector<std::string>& decryped_states)
    {
        ...
        const auto res =
          EVMC(mc, es, tx.rw(ctx.cloakTables.tx_results)).run_with_result();
        ct.function.raw_outputs = res.output;

        // == get new states ==
        auto get_new_states_call_data = ct.get_states_call_data(false);
        LOG_DEBUG_FMT(
          "get_new_states_call_data:{}",
          eevm::to_hex_string(get_new_states_call_data));
        MessageCall get_new_states_mc(
          tee_addr, ct.to, get_new_states_call_data);

        auto get_new_states_res =
          EVMC(get_new_states_mc, es, tx.rw(ctx.cloakTables.tx_results))
            .run_with_result();
        LOG_DEBUG_FMT(
          "get_new_states res:{}, {}, {}, {}",
