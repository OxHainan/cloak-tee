...
namespace cloak4ccf
{
    ...
    namespace TeeManager
    {
        struct Account : public eevm::Account
        {
            ...
            crypto::secp256k1::KeyPairPtr get_tee_kp() const
            {
                ...
                return crypto::secp256k1::make_key_pair(kp_it.value());
            }
        ...
        Address get_service_addr(tables::CloakService::Handle* view)
        {
            auto it = view->get();
            if (!it.has_value())
            {
                ...
                throw std::logic_error(
                  "Cloak service contract address not found!");
            }
            return it.value();
        }
        ...
        class State
        {
          public:
            ...
            AccountPtr create()
            {
                ...
                auto kp = crypto::secp256k1::make_key_pair();
            ...
            {
                return State(
                  table.acc.get_views(tx), table.key_pair.get_views(tx));
            }
        ...
        void append_argument(
          std::vector<uint8_t>& code, const std::vector<uint8_t>& arg)
        {
            code.insert(code.end(), arg.begin(), arg.end());
        }

        void prepare(
          kv::Tx& tx, tables::Table& tee_table, const TeePrepare& tee_prepare)
        {
            ...
            LOG_INFO_FMT(
              ...
              eevm::to_hex_string(tee_acc->get_public_Key()));
            auto contractCode =
              eevm::to_bytes(tee_prepare.cloakServiceContract);
            append_argument(contractCode, encoder.encode());
            const auto contractAddress = eevm::generate_address(
              tee_acc->get_address(), tee_acc->get_nonce());
            LOG_INFO_FMT(
              "Cloak service address {}", eevm::to_hex_string(contractAddress));
            Ethereum::MessageCall mc(tee_acc->get_address(), contractCode);
            auto signed_data = evm4ccf::sign_eth_tx(
              tee_acc->get_tee_kp(), mc, tee_acc->get_nonce());
            Utils::cloak_agent_log(
              "register_tee_addr", eevm::to_hex_string(signed_data));
