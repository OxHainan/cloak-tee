...
namespace Ethereum
{
    struct Transaction : public evm4ccf::EthereumTransactionWithSignature
    {
        ...
        bool validate(
          const uint256_t& blockHash, const uint256_t& timestamp) const
        {
            ...
            auto m =
              ByteString(data.begin() + 4, data.begin() + MESSAGE_LENGTH + 4);
            return blockHash ==
              eevm::to_uint256(
                     ByteString(m.begin(), m.begin() + 32).data(),
                     32u,
                     false) &&
              timestamp ==
              eevm::to_uint256(
                ByteString(m.begin() + 32, m.end()).data(), 32u, false) &&
