...
#include "host/load_monitor.h"
#include "host/node_connections.h"
#include "host/process_launcher.h"
#include "host/rpc_connections.h"
#include "host/sig_term.h"
...
int main(int argc, char** argv)
{
    ...
    app.add_option("-c,--config", config_file_path, "Path to JSON configuration file")->check(CLI::ExistingFile);

    bool check_config_only = false;
    app.add_flag("--check", check_config_only, "Verify configuration file and exit");
    ...
    try {
        ...
        throw std::logic_error(fmt::format("Error parsing configuration file {}: {}", config_file_path, e.what()));
    }
        ...
        if (config.command.type == StartType::Start) {
            if (files::exists(config.ledger.directory)) {
                throw std::logic_error(
                    fmt::format("On start, ledger directory should not exist ({})", config.ledger.directory));
            }
            ...
            recovery_threshold = config.command.start.service_configuration.recovery_threshold;
    ...
    asynchost::TimeBoundLogger::default_max_time = config.slow_io_logging_threshold;
    ...
    ringbuffer::BufferDef to_enclave_def{to_enclave_buffer.data(), to_enclave_buffer.size(), &to_enclave_offsets};
    if (!ringbuffer::Const::find_acceptable_sub_buffer(to_enclave_def.data, to_enclave_def.size)) {
        LOG_FATAL_FMT("Unable to construct valid inbound buffer of size {}", buffer_size);
        return static_cast<int>(CLI::ExitCodes::ValidationError);
    }
    ...
    ringbuffer::BufferDef
        from_enclave_def{from_enclave_buffer.data(), from_enclave_buffer.size(), &from_enclave_offsets};
    if (!ringbuffer::Const::find_acceptable_sub_buffer(from_enclave_def.data, from_enclave_def.size)) {
        LOG_FATAL_FMT("Unable to construct valid outbound buffer of size {}", buffer_size);
        return static_cast<int>(CLI::ExitCodes::ValidationError);
    }
    ...
    oversized::WriterConfig writer_config{config.memory.max_fragment_size, config.memory.max_msg_size};
...

    {
        ...
        asynchost::HandleRingbuffer handle_ringbuffer(1ms, bp, circuit.read_from_inside(), non_blocking_factory);
        ...
        asynchost::LFSFileHandler lfs_file_handler(writer_factory.create_writer_to_inside());
        ...
        auto [node_host, node_port] = cli::validate_address(config.network.node_to_node_interface.bind_address);
        ...
        config.network.node_to_node_interface.bind_address = ccf::make_net_address(node_host, node_port);
        ...
        if (!config.output_files.node_to_node_address_file.empty()) {
            ...
            resolved_node_address[node_to_node_interface_name] = config.network.node_to_node_interface.bind_address;
            files::dump(nlohmann::json(resolved_node_address).dump(), config.output_files.node_to_node_address_file);
        }

        asynchost::ConnIDGenerator idGen;

        asynchost::RPCConnections<asynchost::TCP> rpc(writer_factory, idGen, config.client_connection_timeout);
        ...
        asynchost::RPCConnections<asynchost::UDP> rpc_udp(writer_factory, idGen, config.client_connection_timeout);
        ...
        for (auto& [name, interface] : config.network.rpc_interfaces) {
            ...
            LOG_INFO_FMT("Registering RPC interface {}, on {} {}:{}", name, interface.protocol, rpc_host, rpc_port);
            if (interface.protocol == "udp") {
                rpc_udp.listen(0, rpc_host, rpc_port, name);
            } else {
                rpc.listen(0, rpc_host, rpc_port, name);
            }
            LOG_INFO_FMT("Registered RPC interface {}, on {} {}:{}", name, interface.protocol, rpc_host, rpc_port);
            ...
            auto [pub_host, pub_port] = cli::validate_address(interface.published_address);
            if (pub_port == "0") {
                pub_port = rpc_port;
        ...
        if (!config.output_files.rpc_addresses_file.empty()) {
            files::dump(nlohmann::json(resolved_rpc_addresses).dump(), config.output_files.rpc_addresses_file);
        }
        ...
        if (config.node_data_json_file.has_value()) {
            startup_config.node_data = files::slurp_json(config.node_data_json_file.value());
        }

        auto startup_host_time = std::chrono::system_clock::now();
        LOG_INFO_FMT("Startup host time: {}", startup_host_time);

        startup_config.startup_host_time = ds::to_x509_time_string(startup_host_time);
        ...
        if (config.command.type == StartType::Start) {
            ...
            for (const auto& constitution_path : config.command.start.constitution_files) {
                ...
                startup_config.start.constitution += files::slurp_string(constitution_path);
            }
            startup_config.start.service_configuration = config.command.start.service_configuration;
            startup_config.start.service_configuration.recovery_threshold = recovery_threshold;
            ...
            LOG_INFO_FMT("Creating new node - join existing network at {}", config.command.join.target_rpc_address);
            startup_config.join.target_rpc_address = config.command.join.target_rpc_address;
            startup_config.join.retry_timeout = config.command.join.retry_timeout;
            startup_config.join.service_cert = files::slurp(config.command.service_certificate_file);
        ...
        if (config.command.type == StartType::Join || config.command.type == StartType::Recover) {
            ...
            if (snapshot_file.has_value()) {
                ...
                LOG_INFO_FMT(
                    "Found latest snapshot file: {} (size: {})", snapshot, startup_config.startup_snapshot.size());
            } else {
                LOG_INFO_FMT("No snapshot found: Node will replay all historical transactions");
            }
        }

        if (config.consensus.type == ConsensusType::BFT) {
#ifdef ENABLE_BFT
            LOG_INFO_FMT("Selected consensus BFT is experimental in {}", ccf::ccf_version);
#else
            LOG_FAIL_FMT("Selected consensus BFT is not supported in {}", ccf::ccf_version);
#endif
        }
        ...
        if (create_status != CreateNodeStatus::OK) {
            LOG_FAIL_FMT("An error occurred when creating CCF node: {}", create_node_result_to_str(create_status));
        ...
        LOG_INFO_FMT("Output self-signed node certificate to {}", config.output_files.node_certificate_file);

        if (config.command.type == StartType::Start || config.command.type == StartType::Recover) {
            files::dump(service_cert, config.command.service_certificate_file);
            LOG_INFO_FMT("Output service certificate to {}", config.command.service_certificate_file);
        }
