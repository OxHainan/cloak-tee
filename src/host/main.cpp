...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        LOG_INFO_FMT(
            "Gateway endpoint: {}", ws_config->remote_endpoint()->endpoint());
    ...
    try {
        if (config.command.type == StartType::Start) {
            ...
            // Count members with public encryption key as only these members
            // will be handed a recovery share. Note that it is acceptable to
            // start a network without any member having a recovery share. The
            // service will check that at least one recovery member is added
            // before the service can be opened.
            ...
            if (recovery_threshold == 0) {
                ...
                throw std::logic_error(fmt::format(
                    "Recovery threshold ({}) cannot be greater than total "
                    "number ({})"
    ...
    ringbuffer::BufferDef to_enclave_def{
        to_enclave_buffer.data(),
        to_enclave_buffer.size(),
        &to_enclave_offsets};
    ...
    if (!ringbuffer::Const::find_acceptable_sub_buffer(
        ...
        LOG_FATAL_FMT(
            "Unable to construct valid outbound buffer of size {}",
            buffer_size);
        return static_cast<int>(CLI::ExitCodes::ValidationError);
    }
    ...
    oversized::WriterFactory
        writer_factory(non_blocking_factory, writer_config);
...

    {
        ...
        asynchost::RPCConnections<asynchost::TCP>
            rpc(writer_factory, idGen, config.client_connection_timeout);
        ...
        asynchost::RPCConnections<asynchost::UDP>
            rpc_udp(writer_factory, idGen, config.client_connection_timeout);
        ...
        for (auto& [name, interface] : config.network.rpc_interfaces) {
            ...
            resolved_rpc_addresses[name] =
                fmt::format("{}:{}", rpc_host, rpc_port);
        ...
        if (config.command.type == StartType::Start) {
            ...
            startup_config.join.target_rpc_address =
                config.command.join.target_rpc_address;
            startup_config.join.retry_timeout =
            ...
            LOG_FATAL_FMT(
                "Start command should be start|join|recover. Exiting.");
        }
...
#ifdef ENABLE_BFT
            LOG_INFO_FMT(
                "Selected consensus BFT is experimental in {}",
                ccf::ccf_version);
#else
            LOG_FAIL_FMT(
                "Selected consensus BFT is not supported in {}",
                ccf::ccf_version);
#endif
        }
        ...
        auto enclave_thread_start = [&]() {
            try {
                ...
                // This exception should be rethrown, probably aborting the
                // process, but we sleep briefly to allow more outbound messages
                // to be processed. If the enclave sent logging messages, it is
                // useful to read and print them before dying.
        ...
        // Start threads which will ECall and process messages inside the
        // enclave
    ...
    LOG_INFO_FMT(
        "Ran an extra {} cleanup iteration(s)", 100 - close_iterations);
