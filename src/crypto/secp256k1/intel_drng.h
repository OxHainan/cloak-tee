...
namespace crypto::secp256k1
{
    ...
    class IntelDRNG : public Entropy
    {
      private:
        ...
        static void cpuid(
          cpuid_t* info, unsigned int leaf, unsigned int subleaf)
        {
            asm volatile("cpuid"
                         : "=a"(info->eax),
                           "=b"(info->ebx),
        ...
        static unsigned int rdrand_get_bytes(
          unsigned int n, unsigned char* dest)
        {
            ...
            else
            {
                ...
                lblock =
                  ((n - lhead) &
                   ~(unsigned int)7); // cwinter: this bit is/as buggy in
                                      // the Intel examples.
            }
      ...
      public:
        ...
        uint64_t random64() override
        {
            ...
            if (
              rdrand_get_bytes(len, reinterpret_cast<unsigned char*>(&rnd)) <
              len)
            {
                throw std::logic_error("Couldn't create random data");
            }
