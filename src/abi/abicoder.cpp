...
UINT8ARRAY abicoder::uint256Coder(size_t size) { return UintNumber().encode(to_string(size)); }
...
// handle Array type
void abicoder::paramCoder(vector<void*>& coders,
                          const ByteData& name,
                          const ByteData& _type,
                          const vector<ByteData>& value) {
    auto [type, length, boolean] = Parsing(_type).result();
    if (boolean) {
        // array
        size_t len = length > 1 ? length : value.size();
...
void abicoder::paramCoder(
    vector<void*>& coders, const ByteData& name, const ByteData& type, const ByteData& value, int length) {
