...
namespace abicoder
{
inline std::vector<uint8_t> sub_vector(
    ...
    const size_t& offset = 32u)
{
    if (offset > inputs.size()) {
        throw ABIException(fmt::format(
            "Insufficient array length, want [{}] get [{}]",
            offset,
            inputs.size()));
    }

    return std::vector<
        uint8_t>(inputs.begin() + begin, inputs.begin() + offset);
}
    ...
    bool signal = true)
{
    ...
    for (size_t i = offset, x = value.size() - 1; i < result.size() && x >= 0;
         i++, x--) {
        result.at(i) = value.at(x);
    }
}
...
inline void to_array(
    std::vector<uint8_t>& result, const uint8_t& value, size_t offset)
{
    result.at(offset) = value;
}

inline std::vector<uint8_t> to_bytes(
    const std::string& _s, size_t offset = 0, bool boolean = true)
{
    auto s = eevm::strip(_s);
    if (s.size() > 64) {
        throw ABIException(
            fmt::format("Invalid length, want {} but get {}", 32, s.size()));
    }
...
inline std::vector<uint8_t> bytes_strip(const std::string& src)
{
...
inline nlohmann::json make_array_type(
    const nlohmann::json& j, const std::vector<size_t>& num = {})
{
    if (!num.size())
...
inline nlohmann::json make_bytes_array(
    const size_t& len = 0, const std::vector<size_t>& num = {})
{
    auto type = common_type("bytes", len);
    return make_array_type(type, num);
}

inline nlohmann::json make_common_array(
    const std::string& t, const std::vector<size_t>& num = {})
{
...
inline std::vector<std::string> split_abi_data(const std::vector<uint8_t>& data)
{
    ...
    for (size_t i = 0; i < count; i++) {
        res.push_back(eevm::to_hex_string(
            {data.begin() + i * 32, data.begin() + (i + 1) * 32}));
    }
    if (data.size() % 32) {
        res.push_back(
            eevm::to_hex_string({data.begin() + count * 32, data.end()}));
    }
    return res;
}
...
inline size_t get_static_array_size(const nlohmann::json& type)
{
    ...
    return type["len"].get<size_t>() *
        get_static_array_size(type["value_type"]);
}
