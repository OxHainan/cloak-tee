...
namespace abicoder {
inline double alignSize(size_t size) { return 32 * (ceil(size / 32.0)); }
...
// template <size_t N>
inline void to_array(UINT8ARRAY &result, const uint8_t &value, size_t offset) { result.at(offset) = value; }
...
class UintNumber {
 public:
    UintNumber() {}
    UintNumber(size_t _size, bool _signal) : size(_size), signal(_signal) {
        name = (signal ? "int" : "uint") + to_string(size * 8);
    }

    UINT8ARRAY encode(const ByteData &value) { return encode(eevm::to_uint256(value)); }
...
class CoderNumber : public Coder {
 ...
 public:
    CoderNumber() {}
    CoderNumber(size_t _size, bool _signal) : size(_size), signal(_signal) {
        name = (signal ? "int" : "uint") + to_string(size * 8);
    }

    void setValue(const ByteData &_value) { value = _value; }

    UINT8ARRAY encode() { return UintNumber().encode(value); }
    bool getDynamic() const { return Dynamic; }
};

class CoderAddress : public Coder {
 public:
    explicit CoderAddress(ByteData _name) : name(_name) {}
    void setValue(const ByteData &_value) { value = _value; }
    UINT8ARRAY encode() {
...
class CoderBoolean : public Coder {
 public:
    explicit CoderBoolean(ByteData _name) : name(_name) { uint256CoderBool = UintNumber(32, true); }
    void setValue(const ByteData &_value) { value = _value; }
    UINT8ARRAY encode() { return uint256CoderBool.encode(value == "1" ? true : false); }
...
class CoderString : public Coder {
 public:
    explicit CoderString(ByteData _name, bool _dynamic = true) : name(_name), Dynamic(_dynamic) {}
    void setValue(const ByteData &_value) { value = _value; }
    UINT8ARRAY encode() {
...
class CoderDynamicBytes : public Coder {
 public:
    CoderDynamicBytes() {}
    explicit CoderDynamicBytes(ByteData _name, bool _dynamic = true) : name(_name), Dynamic(_dynamic) {}
    void setValue(const ByteData &_value) { value = _value; }
...
class CoderFixedBytes : public Coder {
 public:
    CoderFixedBytes() {}
    explicit CoderFixedBytes(size_t _length) : length(_length) { name = "bytes" + to_string(length); }
    void setValue(const ByteData &_value) { value = _value; }
...
class CoderArray : Coder {
 public:
    ...
    void setValue(const vector<ByteData> &_value) { value = _value; }
...
enum paramsType { ADDRESS, UINT, INT, BYTES, STRING, BOOL };
