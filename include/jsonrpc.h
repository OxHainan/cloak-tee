...
#define XX(Name, Value) \
    ...
    switch (ec) { XX_STANDARD_ERROR_CODES }
...
inline std::optional<jsonrpc::Pack> detect_pack(const std::vector<uint8_t>& input) {
    if (input.size() == 0) {
        return {};
    }
...
template <typename ErrorEnum>
inline std::pair<bool, nlohmann::json> error(ErrorEnum error_code, const std::string msg = "") {
...
template <typename ErrorEnum>
inline nlohmann::json error_response(SeqNo id, ErrorEnum error_code, const std::string& msg) {
    nlohmann::json j;
    j[JSON_RPC] = RPC_VERSION;
    j[ID] = id;
