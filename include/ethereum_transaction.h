...
inline size_t to_ethereum_recovery_id(size_t rec_id) {
    ...
    if (rec_id > 1) {
        throw std::logic_error(
            fmt::format("Ethereum only accepts finite curve coordinates, {} represents an "
                        "infinite value",
                        rec_id));
    }
...
inline size_t from_ethereum_recovery_id(size_t v) {
    ...
    if (v < min_valid_v) {
        throw std::logic_error(
            fmt::format("Expected v to encode a valid chain ID (must be at least {}), but is "
                        "{}",
                        min_valid_v,
                        v));
    }
...
inline std::vector<uint8_t> public_key_asn1(mbedtls_pk_context* raw_ctx) {
    ...
    if (written < 0) {
        throw std::logic_error("mbedtls_pk_write_pubkey: " + tls::error_string(written));
    }
...
inline EthereumTransactionWithSignature sign_transaction(tls::KeyPair_k1Bitcoin& kp,
